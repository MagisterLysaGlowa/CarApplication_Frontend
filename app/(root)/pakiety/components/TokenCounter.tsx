...
export const getPolishTokenWord = (count: number): string => {
  const lastDigit = count % 10;
  const lastTwoDigits = count % 100;

  if (count === 1) return "kluczyk";
  if (lastTwoDigits >= 12 && lastTwoDigits <= 14) return "kluczyków";
  if (lastDigit >= 2 && lastDigit <= 4) return "kluczyki";
  return "kluczyków";
};
...
const TokenCounter = ({ user }: TokenCounterProps) => {
    ...
    const fetchTokensCount = async () => {
      ...
      if (user) {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_BACKEND_URL}/Token/${user.data.userId}`
        );
  ...
  return (
    <div className="flex items-center bg-white rounded-[20px] p-3 px-6 gap-2 mx-6 lg:mx-auto">
      ...
      <p className="lg:text-[16px] text-[14px] text-nowrap font-[400] sm:block hidden">
