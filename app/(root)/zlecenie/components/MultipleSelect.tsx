...
const MultipleSelect = ({
  ...
  const isDoorsCountArray = (data: any): data is DoorsCountType[] => {
    return (
      Array.isArray(data) &&
      data.length > 0 &&
      data.every((item) => "doorsQuantity" in item)
    );
  };
            ...
            {selectedOptions.length > 0 ? (
              <>
                {selectedOptions.map((item, index) => {
                  if (name == "bodyType" && isBodyTypeArray(selectOptionData)) {
                    return `${
                  ...
                  if (name == "fuelType" && isFuelTypeArray(selectOptionData)) {
                    return `${
                      selectOptionData.find((so) => so.fuelTypeId == item)!.name
                    }${index < selectedOptions.length - 1 ? ", " : ""}`;
                  }
                  if (name == "gearBox" && isGearBoxArray(selectOptionData)) {
                    return `${
