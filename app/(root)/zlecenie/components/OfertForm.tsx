...
const OfertForm = () => {
  ...
  const createOfferMutation = useMutation({
    mutationFn: async (data: any) => {
      ...
      setLoading(true);
      const pendingToastId = toast.loading("Trwa dodawanie oferty...");
      ...
      try {
        ...
        toast.update(pendingToastId, {
          render: "Pomyślnie dodano ofertę!",
          type: "success",
          isLoading: false,
          autoClose: 3000,
          onClose: () => {
            setLoading(false);
            router.push("/potwierdz-zlecenie");
          },
        });
        ...
        setLoading(false);
        toast.update(pendingToastId, {
          render: "Nie udało się dodać oferty!",
          type: "error",
          isLoading: false,
          autoClose: 3000,
        });
    ...
    <form
      ...
      onSubmit={handleSubmit((data) => {
        if (
          data.horsePowerFrom &&
          data.horsePowerTo &&
          data.horsePowerFrom > data.horsePowerTo
        ) {
          toast.error("Nieprawidłowa liczba koni");
          return;
        }
                  ...
                  <MultipleSelect
                    ...
                    name={
                      item.name == "bodyTypeId"
                        ...
                        ? "doorsCount" // This mapping is correct but code didn't recognize it properly
                        : item.name == "gearBoxId"
                        ? "gearBox"
                    ...
                    selectOptionData={
                      item.name == "bodyTypeId"
                        ...
                        ? fuelTypeDb
                        : item.name == "doorsCountId"
                        ? doorsCountDb // This is correct but there might be an issue with the data itself
                        : item.name == "gearBoxId"
                        ? gearBoxDb
                        : item.name == "carStateId"
                        ? carStateDb
